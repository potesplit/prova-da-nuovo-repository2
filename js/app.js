...
function refreshSpesaNERighe() {
  const box = document.querySelector("#f-spesa-ne-righe");
  if (!box) return;
  const preset = {};
  box.querySelectorAll(".spesa-ne-row").forEach(row => {
    if (row.querySelector(".sne-check").checked) {
      preset[row.dataset.nome] = row.querySelector(".sne-quota").value;
    }
  });
  buildSpesaNERighe(preset);
}
...
function renderListaSpese() {
  ...
  STATE.spese.forEach((s, i) => {
    ...
    row.innerHTML = `
      <div class="list-main">${escapeHtml(s.nome)} — ${escapeHtml(s.descrizione)} <strong>${euro(s.importo)}</strong><br><span class="list-sub">Partecipanti: ${escapeHtml(part)}</span></div>
      <div class="list-actions">
        <button class="btn-icon edit" type="button">Modifica</button>
        <button class="btn-icon delete" type="button">Elimina</button>
      </div>`;
    row.querySelector(".edit").addEventListener("click", () => isNE ? modificaSpesaNE(i) : modificaSpesa(i));
    row.querySelector(".delete").addEventListener("click", async () => {
...
// viene calcolato automaticamente come somma delle quote inserite.
let editingSpesaNEIndex = null;

function buildSpesaNERighe(preselezionati) {
  const box = document.querySelector("#f-spesa-ne-righe");
  if (!box) return;
  const persone = [...STATE.persone].sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
  ...
  box.innerHTML = persone.map(p => {
    ...
    return `<div class="spesa-ne-row" data-nome="${escapeHtml(p)}">
      <label class="chk"><input type="checkbox" class="sne-check" ${checked}> ${escapeHtml(p)}</label>
      <input type="number" step="0.01" class="sne-quota" placeholder="Quota €" value="${valore}">
      <span></span>
    </div>`;
  }).join("");
  aggiornaTotaleSpesaNE();
  box.querySelectorAll(".sne-quota, .sne-check").forEach(inp => inp.addEventListener("input", aggiornaTotaleSpesaNE));
  box.querySelectorAll(".sne-check").forEach(cb => cb.addEventListener("change", aggiornaTotaleSpesaNE));
}
...
function leggiQuoteSpesaNE() {
  ...
  const quote = {};
  righe.forEach(row => {
    const checked = row.querySelector(".sne-check").checked;
    if (!checked) return;
...
function aggiornaTotaleSpesaNE() {
  ...
  if (box) box.textContent = `Totale spesa (somma delle quote): ${euro(tot)}`;
}

function modificaSpesaNE(i) {
  const s = STATE.spese[i];
  editingSpesaNEIndex = i;
  document.querySelector("#f-spesa-ne-chi").value = s.nome;
  document.querySelector("#f-spesa-ne-desc").value = (s.descrizione || "").replace(/\s*\[NE\]\s*$/, "");
  buildSpesaNERighe(s.quote || {});
...
async function submitSpesaNE(e) {
  e.preventDefault();
  const nome = document.querySelector("#f-spesa-ne-chi").value;
  let descrizione = document.querySelector("#f-spesa-ne-desc").value.trim();
  if (!nome || !descrizione) { setStatus("#status-spesa-ne", "Compila chi ha anticipato e la descrizione.", true); return; }
  if (!validaOAvvisa(descrizione, "Descrizione")) return;
  ...
  const importo = Object.values(quote).reduce((a, b) => a + b, 0);
  if (importo <= 0) { setStatus("#status-spesa-ne", "La somma delle quote deve essere maggiore di zero.", true); return; }

  descrizione = descrizione.replace(/\s*\[NE\]\s*$/, "") + " [NE]";

  try {
    const nuova = { nome, descrizione, importo, partecipanti, quote };
    let nuovoElenco;
    let msg;
    if (editingSpesaNEIndex !== null) {
      nuovoElenco = STATE.spese.map((s, i) => i === editingSpesaNEIndex ? nuova : s);
    ...
    await GH.writeJSON("data/spese.json", nuovoElenco, msg);
    setStatus("#status-spesa-ne", (editingSpesaNEIndex !== null ? "Spesa modificata!" : "Spesa aggiunta!") + " Ricarico i dati…", false);
